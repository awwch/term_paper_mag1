...
nouns = []
i = 0
_dict = pd.read_csv( "C:\\Users\\Ania\\Desktop\\kursach\\BTS\\output1.csv", header=0, 
 delimiter=";")
...
for definition in definitions:
    ...
    try:
        sentences += def_to_sentences(definition, tokenizer)
    except KeyError:
        continue
...
f = open('nouns.csv','w',encoding = 'utf-8')
for element in c:    
...
for sentence in sentences:
    ...
    for word in k_words:
        ...
        for s in sentence:
            if len(s)>0 and str_key(s[0]) == word:
                k_def = sentence[0]
...
f = open('абажур.txt','w',encoding = 'utf-8')
for dice in dice_dict:
    if str_value(dice) != '0.0':
        f.write(str(dice)+'\n')
f.close()

#c = Counter(all_words_normalized).most_common()
